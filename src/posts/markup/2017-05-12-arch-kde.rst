...
SSH
===
Install ``plasma-workspace-agent-ssh`` and add ``AddKeysToAgent yes`` to ``~/.ssh/config``

.. export SSH_ASKPASS='/usr/bin/ksshaskpass'

This will only enable the agent in your plasma sessions however, so you might want to go for the `systemd user service`_
